...
# Use an absolute path: .githooks/ may live outside REPO_ROOT entirely
# (a centrally installed ~/briterepo/bin/helpers/.githooks shared across
# repos), not just in a vendored copy inside the repo being configured.
if ! git config core.hooksPath "$GITHOOKS_DIR" >/dev/null 2>&1; then
