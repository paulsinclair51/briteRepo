...
#
# Usage: source briterepo/bin/helpers/common_utils.sh
#        source briterepo/bin/helpers/validation_helpers.sh
#
# Functions:
...
#
# Example: validate_shell_format "briterepo/bin/mkbranch"
...
# validate_version_branch: Check if branch is a version branch (v<M>.<m>.0)
#
# Args: $1 = branch name
...
validate_targeted_branch() {
  ...
  if [[ "$branch" =~ ^(dev|fix)/[a-z0-9][a-z0-9-]*-(v[1-9][0-9]?\.(0|[1-9][0-9]?)\.0)$ ]]; then
    return 0
  fi
  return 1
}
...
validate_branch_name() {
  ...
  if [[ -n "$context" ]]; then
    ...
    log_error "  - 'main'" >&2
    log_error "  - Version: v<M>.<m>.0 (e.g., v1.0.0)" >&2
    log_error "  - Targeted: dev/<desc>-v<M>.<m>.0 or fix/<desc>-v<M>.<m>.0" >&2
