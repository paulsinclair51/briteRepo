...
mkdir -p "$WORK/briterepo/bin" "$WORK/briterepo/bin/helpers" "$WORK/reports"
cp "$REPO_ROOT/briterepo/bin/pushup" "$WORK/briterepo/bin/pushup"
cp "$REPO_ROOT/briterepo/bin/helpers/common.sh" \
  "$REPO_ROOT/briterepo/bin/helpers/git_helpers.sh" \
  "$REPO_ROOT/briterepo/bin/helpers/report_helpers.sh" \
  "$REPO_ROOT/briterepo/bin/helpers/history_log.sh" "$WORK/briterepo/bin/helpers/"
...
cat > "$WORK/briterepo/bin/helpers/pushup_parent.sh" <<'EOF'
#!/usr/bin/env bash
set -e
dry_run=false
error_run=false
...
EOF
cat > "$WORK/briterepo/bin/helpers/push_command.sh" <<'EOF'
#!/usr/bin/env bash
bt_push_init() {
  PUSH_WORKFLOW_ARGS=()
...
EOF
cat > "$WORK/briterepo/bin/helpers/pulldown_workflow.sh" <<'EOF'
#!/usr/bin/env bash
bt_pulldown_init() {
...
chmod +x "$WORK/briterepo/bin/"{push,chbranch,pull} \
  "$WORK/briterepo/bin/helpers/"{pulldown_workflow.sh,push_command.sh,pushup_parent.sh}
...
EOF
chmod +x "$WORK/briterepo/bin/helpers/pulldown_workflow.sh"
...
chmod +x "$WORK/briterepo/bin/chbranch" \
  "$WORK/briterepo/bin/helpers/pulldown_workflow.sh"
