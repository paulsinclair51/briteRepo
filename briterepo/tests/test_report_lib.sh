...
report_test_init() {
  ...
  cat > "$WORK/briterepo/bin/helpers/ckstyle.sh" <<'EOF'
#!/usr/bin/env bash
bt_ckstyle() {
  printf '%s\n' "$@" > style-args.txt
...
EOF
  cat > "$WORK/briterepo/bin/helpers/branch_status.sh" <<'EOF'
#!/usr/bin/env bash
...
EOF
  chmod +x "$WORK/briterepo/bin/report" "$WORK/briterepo/bin/helpers/ckstyle.sh"
