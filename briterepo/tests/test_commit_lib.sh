...
HISTORY_LOG_HELPER_SRC="$REPO_ROOT/briterepo/bin/helpers/history_log.sh"
...
commit_test_init() {
  ...
  mkdir -p "$WORK/briterepo/bin" "$WORK/briterepo/bin/helpers" "$WORK/config" "$WORK/reports"
  ...
  cp "$HISTORY_LOG_HELPER_SRC" "$WORK/briterepo/bin/helpers/history_log.sh"
