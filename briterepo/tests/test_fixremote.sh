...
# test_fixremote.sh - smoke tests for briterepo/bin/fixremote
#
...
FIXREMOTE_SRC="$REPO_ROOT/briterepo/bin/fixremote"
...
HEALTH_REPORT_HELPER_SRC="$REPO_ROOT/briterepo/bin/helpers/health_report.sh"
...
make_runner_repo() {
  local root="$1"
  mkdir -p "$root/briterepo/bin" "$root/briterepo/bin/helpers" "$root/config" "$root/reports"

  cp "$FIXREMOTE_SRC" "$root/briterepo/bin/fixremote"
  cp "$COMMON_HELPER_SRC" "$root/briterepo/bin/helpers/common.sh"
  cp "$GIT_HELPER_SRC" "$root/briterepo/bin/helpers/git_helpers.sh"
  cp "$REPORT_HELPER_SRC" "$root/briterepo/bin/helpers/report_helpers.sh"
  cp "$HEALTH_REPORT_HELPER_SRC" "$root/briterepo/bin/helpers/health_report.sh"
  cp "$CKROLE_HELPER_SRC" "$root/briterepo/bin/helpers/ckrole.sh"
  cp "$HEALTH_REPORT_HELPER_SRC" "$root/briterepo/bin/helpers/health_report.sh"
  chmod +x "$root/briterepo/bin/fixremote"
...
# 1) Help output
rc=$(run_capture "$TMPDIR/help.out" bash "$RUNNER/briterepo/bin/fixremote" -h)
...
rc=$(run_capture "$TMPDIR/missing.out" bash "$RUNNER/briterepo/bin/fixremote")
...
rc=$(run_capture "$TMPDIR/preflight.out" env GITHUB_ACTOR="paulsinclair51" bash "$RUNNER/briterepo/bin/fixremote" "$SOURCE_CLONE")
...
rc=$(run_capture "$TMPDIR/t0.out" env GITHUB_ACTOR="paulsinclair51" bash "$RUNNER/briterepo/bin/fixremote" -x -t 0 "$SOURCE_CLONE")
[[ "$rc" -eq 3 ]] || fail "fixremote -x -t 0 should exit 3 (got $rc)"
report_r0="$(latest_recovery_report "$RUNNER")"
[[ -f "$report_r0" ]] || fail "expected recovery report for -x -t 0 run"
assert_contains "Cannot execute recovery with -t 0" "$report_r0"
...
rc=$(run_capture "$TMPDIR/execute.out" env GITHUB_ACTOR="paulsinclair51" bash "$RUNNER/briterepo/bin/fixremote" -x "$SOURCE_CLONE")
