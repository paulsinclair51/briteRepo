...
Runs a test suite for briterepo/bin/gendocs and its helper scripts
briterepo/bin/helpers/genpdf.sh and briterepo/bin/helpers/gendocx.sh.
...
gendocs_script="$repo_root/briterepo/bin/gendocs"
genpdf_helper="$repo_root/briterepo/bin/helpers/genpdf.sh"
gendocx_helper="$repo_root/briterepo/bin/helpers/gendocx.sh"
common_helper="$repo_root/briterepo/bin/helpers/common.sh"
git_helper="$repo_root/briterepo/bin/helpers/git_helpers.sh"
brand_name=""
brand_file="$repo_root/docs/branding/Logo_with_BrandName.svg"
if [[ -f "$brand_file" ]]; then
  brand_name="$(sed -n 's/.*<text[^>]*>\([^<][^<]*\)<\/text>.*/\1/p' \
    "$brand_file" | head -n 1)"
fi
...
make_gendocs_repo() {
  ...
  mkdir -p "$repo/briterepo/bin" "$repo/briterepo/bin/helpers" \
    "$repo/docs/md" "$repo/docs/pdf" "$repo/docs/docx" "$repo/logs"
  cp "$gendocs_script" "$repo/briterepo/bin/gendocs"

  cat > "$repo/briterepo/bin/helpers/genpdf.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  ...
  cat > "$repo/briterepo/bin/helpers/gendocx.sh" <<'EOF'
#!/usr/bin/env bash
set -euo pipefail
...
EOF
  ...
  chmod +x "$repo/briterepo/bin/gendocs" \
    "$repo/briterepo/bin/helpers/genpdf.sh" \
    "$repo/briterepo/bin/helpers/gendocx.sh"
  cp "$common_helper" "$repo/briterepo/bin/helpers/common.sh"
  cp "$git_helper" "$repo/briterepo/bin/helpers/git_helpers.sh"
  mkdir -p "$repo/config"
...
make_genpdf_helper_repo() {
  ...
  mkdir -p "$repo/briterepo/bin/helpers" "$repo/docs/branding" \
    "$repo/in/docs/branding" "$repo/in" "$repo/out" "$repo/bin"
  cp "$genpdf_helper" "$repo/briterepo/bin/helpers/genpdf.sh"
...
EOF
  ...
  chmod +x "$repo/briterepo/bin/helpers/genpdf.sh" "$repo/bin/pandoc"
  printf '%s\n' "$repo"
}
...
make_gendocx_helper_repo() {
  ...
  mkdir -p "$repo/briterepo/bin/helpers" "$repo/bin" "$repo/in" "$repo/out"
  cp "$gendocx_helper" "$repo/briterepo/bin/helpers/gendocx.sh"
...
EOF
  ...
  chmod +x "$repo/briterepo/bin/helpers/gendocx.sh" "$repo/bin/libreoffice"
  printf '%s\n' "$repo"
}
...
test_gendocs_help() {
  ...
  bash "$repo/briterepo/bin/gendocs" -h > "$tmpdir/gendocs_help.out"
...
EOF
  ...
  bash "$repo/briterepo/bin/gendocs" > "$tmpdir/gendocs_default.out"
...
EOF
  ...
  bash "$repo/briterepo/bin/gendocs" -n -p > \
...
test_gendocs_toolcheck() {
  ...
  bash "$repo/briterepo/bin/gendocs" -T > "$tmpdir/gendocs_toolcheck.out"
...
test_gendocs_docx_only_pdf_source() {
  ...
  bash "$repo/briterepo/bin/gendocs" -d \
...
test_gendocs_docx_target_collision() {
  ...
  rc=$(run_capture "$tmpdir/gendocs_docx_collision.out" \
    bash "$repo/briterepo/bin/gendocs" -d "$repo/docs/md")
...
test_gendocs_prerequisites() {
  ...
  rc=$(run_capture "$tmpdir/gendocs_role.out" \
    bash "$repo/briterepo/bin/gendocs" -n)
...
test_gendocs_missing_helper() {
  ...
  rm -f "$repo/briterepo/bin/helpers/gendocx.sh"

  rc=$(run_capture "$tmpdir/gendocs_missing_helper.out" \
    bash "$repo/briterepo/bin/gendocs")
...
test_gendocs_genpdf_helper_reorders_first_png() {
  ...
  [[ -n "$brand_name" ]] || { echo "SKIP: brand source unavailable"; return 0; }
  ...
  cat > "$input" <<EOF
# Title
![${brand_name} Logo](docs/branding/Logo_with_BrandName.png)

Body line.
EOF

  BRITEREPO_INTERNAL=1 PATH="$repo/bin:$PATH" bash "$repo/briterepo/bin/helpers/genpdf.sh" -e \
  ...
  if grep -Fq "![${brand_name} Logo](" "$emitted"; then
...
test_gendocx_helper_invalid_backend() {
  ...
  rc=$(run_capture "$tmpdir/gendocx_invalid.out" env BRITEREPO_INTERNAL=1 bash \
    "$repo/briterepo/bin/helpers/gendocx.sh" -b invalid \
...
test_gendocx_helper_libreoffice_backend() {
  ...
  BRITEREPO_INTERNAL=1 PATH="$repo/bin:$PATH" bash "$repo/briterepo/bin/helpers/gendocx.sh" -b \
