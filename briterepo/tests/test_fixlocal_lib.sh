...
make_fixture_repo() {
  ...
  mkdir -p "$repo/briterepo/bin" "$repo/briterepo/bin/helpers" "$repo/reports" "$repo/config"
  ...
  cp "$HEALTH_REPORT_HELPER_SRC" "$repo/briterepo/bin/helpers/health_report.sh"
  cp "$CKROLE_HELPER_SRC" "$repo/briterepo/bin/helpers/ckrole.sh"
