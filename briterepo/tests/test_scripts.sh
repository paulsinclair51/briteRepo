...
# test_scripts.sh - Run all script smoke tests in briterepo/tests.
#
...
# If split report test files are present, skip the report orchestrator to
# avoid running the same suite twice in this global runner.
if find "$SCRIPT_DIR" -maxdepth 1 -type f -name 'test_report_*.sh' | grep -q .; then
  filtered=()
  for test_file in "${test_files[@]}"; do
    if [[ "$test_file" != "$SCRIPT_DIR/test_report.sh" ]]; then
      filtered+=("$test_file")
    fi
  done
  test_files=("${filtered[@]}")
