...
PUSHUP_SRC="$REPO_ROOT/briterepo/bin/helpers/pushup_parent.sh"
PUSH_SRC="$REPO_ROOT/briterepo/bin/push"
HELPERS_DIR="$REPO_ROOT/briterepo/bin/helpers"
...
cp "$PUSHUP_SRC" "$WORK/briterepo/bin/helpers/pushup_parent.sh"
cp "$PUSH_SRC" "$WORK/briterepo/bin/push"
chmod +x "$WORK/briterepo/bin/helpers/pushup_parent.sh" "$WORK/briterepo/bin/push"
for helper in common.sh git_helpers.sh github_helpers.sh \
  ...
  [[ -f "$HELPERS_DIR/$helper" ]] && \
    cp "$HELPERS_DIR/$helper" "$WORK/briterepo/bin/helpers/$helper"
done
...
FORBIDDENPUSHEOF
  ...
  cp "$HELPERS_DIR/push_workflow.sh" briterepo/bin/helpers/push_workflow.sh
  chmod +x briterepo/bin/push
  echo "feature work" > feature.txt
  "$REAL_GIT" add feature.txt briterepo/bin/push briterepo/bin/helpers/push_workflow.sh
...
rc=$(run_capture "$TMPDIR/skip-e.out" env PATH="$FAKEBIN:$PATH" GITHUB_ACTOR=testowner bash -lc "cd '$WORK' && bash ./briterepo/bin/helpers/pushup_parent.sh --pushup -e")
...
run_pushup() {
  ...
  (
    ...
    env "${envvars[@]}" bash ./briterepo/bin/helpers/pushup_parent.sh --pushup "${pushup_args[@]}"
  ) >"$outfile" 2>&1
  local rc=$?
  set -e
  echo "$rc"
