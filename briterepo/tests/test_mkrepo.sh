...
for item in \
  ...
  "config/version_status.md" ".github/workflows/validate-push.yml" \
...
[[ ! -e "$WORK_DIR/verify-new/briterepo" ]] || \
  fail "mkrepo added a briterepo/ directory; it should be installed separately"
...
[[ -e "$WORK_DIR/verify-legacy/.github/workflows/validate-push.yml" ]] || \
  fail "mkrepo did not add the workflows to the existing repository"
...
# Stale workflow files are removed by the fresh copy
git clone -q "$GH_FAKE_REMOTES/legacy-repo.git" "$WORK_DIR/stale-seed"
echo "stale" > "$WORK_DIR/stale-seed/.github/workflows/obsolete-workflow.yml"
git -C "$WORK_DIR/stale-seed" add -A
git -C "$WORK_DIR/stale-seed" commit -q -m "add stale workflow"
...
[[ ! -e "$WORK_DIR/verify-stale/.github/workflows/obsolete-workflow.yml" ]] || \
  fail "mkrepo did not replace .github/workflows with a fresh copy"
pass "mkrepo replaces .github/workflows with a fresh copy"
