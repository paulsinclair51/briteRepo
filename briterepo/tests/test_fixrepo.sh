...
# test_fixrepo.sh - smoke tests for briterepo/bin/fixrepo
#
...
FIXREPO_SRC="$REPO_ROOT/briterepo/bin/fixrepo"
...
make_fixture_repo() {
  ...
  mkdir -p "$repo/briterepo/bin" "$repo/briterepo/bin/helpers" "$repo/reports"
  cp "$FIXREPO_SRC" "$repo/briterepo/bin/fixrepo"
  ...
  chmod +x "$repo/briterepo/bin/fixrepo"
...
/* fixture */
EOF
    git -C "$repo" add README.md .gitignore include src briterepo reports
...
# Repository Reports
EOF
    git -C "$repo" add README.md briterepo reports
...
# 1) Help output
rc=$(run_capture "$TMPDIR/help.out" bash "$BRITE_REPO/briterepo/bin/fixrepo" -h)
...
rc=$(run_capture "$TMPDIR/remote.out" bash "$REMOTE_REPO/briterepo/bin/fixrepo" -d -t 0)
...
rc=$(run_capture "$TMPDIR/unreachable.out" bash "$UNREACHABLE_REMOTE_REPO/briterepo/bin/fixrepo" -d -t 1)
...
rc=$(run_capture "$TMPDIR/bad-timeout.out" bash "$PLAIN_REPO/briterepo/bin/fixrepo" -t abc)
...
rc=$(run_capture "$TMPDIR/bad-clone.out" bash "$PLAIN_REPO/briterepo/bin/fixrepo" -d -q "$TMPDIR/does-not-exist")
...
rc=$(run_capture "$TMPDIR/gc-fail.out" env PATH="$FAKEBIN:$PATH" bash "$BRITE_FAIL_REPO/briterepo/bin/fixrepo" -q)
