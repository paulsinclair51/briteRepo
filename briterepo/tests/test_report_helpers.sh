...
# shellcheck source=briterepo/bin/helpers/common.sh
source "$REPO_ROOT/briterepo/bin/helpers/common.sh"
# shellcheck source=briterepo/bin/helpers/report_helpers.sh
source "$REPORT_HELPER_SRC"
# shellcheck source=briterepo/bin/helpers/history_log.sh
...
event_note="$(git -C "$DELETE_REPO" notes --ref=briteRepo-workflow show HEAD)"
