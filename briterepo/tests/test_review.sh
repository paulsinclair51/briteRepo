...
# test_review.sh - smoke tests for briterepo/bin/review workflow actions.
#
...
REVIEW_SRC="$REPO_ROOT/briterepo/bin/review"
...
mkdir -p "$WORK/briterepo/bin" "$WORK/briterepo/bin/helpers" "$WORK/config"
cp "$REVIEW_SRC" "$WORK/briterepo/bin/review"
for helper in common.sh git_helpers.sh github_helpers.sh history_log.sh ckrole.sh; do
  cp "$REPO_ROOT/briterepo/bin/helpers/$helper" "$WORK/briterepo/bin/helpers/$helper"
done
chmod +x "$WORK/briterepo/bin/review"
...
clear_gh_log
rc=$(run_capture "$TMPDIR/help.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -h")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/approve.out" none env GITHUB_ACTOR=testapprover bash -c "cd '$WORK' && bash ./briterepo/bin/review approve -- 'Looks good to me.'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/non_contrib.out" none env GITHUB_ACTOR=unknownactor bash -c "cd '$WORK' && bash ./briterepo/bin/review")
...
EOF
clear_gh_log
rc=$(run_capture "$TMPDIR/no_approver.out" none env GITHUB_ACTOR=reviewer1 bash -c "cd '$WORK' && bash ./briterepo/bin/review")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/create_draft.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -T 'WIP Feature'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/create_draft_with_desc.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -T 'WIP Feature' -c 'Initial draft description.'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/description_alias.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review --description 'alias text'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/dry_run_create.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -d -T 'Dry Run Feature' -c 'Dry run description.'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/dry_run_start.out" draft bash -c "cd '$WORK' && bash ./briterepo/bin/review -d -s")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/dry_run_browser.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -d -b")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/browser_open_ready.out" ready bash -c "cd '$WORK' && bash ./briterepo/bin/review -b")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/browser_closed_fallback.out" closed bash -c "cd '$WORK' && bash ./briterepo/bin/review -b")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/browser_no_pr.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -b")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/start_review.out" draft env FAKE_EXISTING_LABELS="old1,old2" bash -c "cd '$WORK' && bash ./briterepo/bin/review -s -T 'Ready Feature' -l 'bug, urgent'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/delete_draft.out" draft bash -c "cd '$WORK' && bash ./briterepo/bin/review --delete")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/delete_browser.out" draft bash -c "cd '$WORK' && bash ./briterepo/bin/review --delete -b")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/delete_start.out" draft bash -c "cd '$WORK' && bash ./briterepo/bin/review --delete -s")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/delete_description.out" draft bash -c "cd '$WORK' && bash ./briterepo/bin/review --delete -c 'x'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/title_whitespace.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -T '   '")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/labels_empty_token.out" none bash -c "cd '$WORK' && bash ./briterepo/bin/review -l 'bug, ,urgent'")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/pr_list_failure.out" none env FAKE_FAIL_PR_LIST=1 bash -c "cd '$WORK' && bash ./briterepo/bin/review")
...
clear_gh_log
rc=$(run_capture "$TMPDIR/non_draft_blocked.out" ready bash -c "cd '$WORK' && bash ./briterepo/bin/review")
