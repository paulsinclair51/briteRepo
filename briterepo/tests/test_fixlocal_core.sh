...
# test_fixlocal_core.sh - core smoke tests for briterepo/bin/fixlocal.
#
...
rc=$(run_capture "$TMPDIR/brite.out" bash "$BRITE_REPO/briterepo/bin/fixlocal")
...
rc=$(run_capture "$TMPDIR/plain.out" bash "$PLAIN_REPO/briterepo/bin/fixlocal" -d)
...
rc=$(run_capture "$TMPDIR/bad-timeout.out" bash "$PLAIN_REPO/briterepo/bin/fixlocal" -t abc)
...
rc=$(run_capture "$TMPDIR/loose.out" bash "$LOOSE_REPO/briterepo/bin/fixlocal" -d)
...
rc=$(run_capture "$TMPDIR/gc-fail.out" env PATH="$FAKEBIN:$PATH" bash "$BRITE_FAIL_REPO/briterepo/bin/fixlocal")
...
rm -f "$MISSING_HELPER_REPO/briterepo/bin/helpers/git_helpers.sh"
rc=$(run_capture "$TMPDIR/missing-helper.out" bash "$MISSING_HELPER_REPO/briterepo/bin/fixlocal" -d)
...
rc=$(run_capture "$TMPDIR/report-path-fail.out" bash "$REPORT_PATH_FAIL_REPO/briterepo/bin/fixlocal" -d)
...
rc=$(run_capture "$TMPDIR/unauthorized.out" env GITHUB_ACTOR="someotheruser" bash "$UNAUTHORIZED_REPO/briterepo/bin/fixlocal" -d)
