...
# test_genpngs.sh - smoke tests for briterepo/bin/genpngs
#
...
GENPNGS_SRC="$REPO_ROOT/briterepo/bin/genpngs"
...
make_fixture_repo() {
  ...
  mkdir -p "$repo/briterepo/bin" "$repo/docs/branding" "$repo/docs/md"
  cp "$GENPNGS_SRC" "$repo/briterepo/bin/genpngs"
  chmod +x "$repo/briterepo/bin/genpngs"

  cat > "$repo/docs/branding/Logo_with_Tagline.svg" <<EOF
<svg xmlns="http://www.w3.org/2000/svg" width="800" height="200" viewBox="0 0 800 200">
  <rect width="800" height="200" fill="#FCFCFD"/>
  <text x="20" y="90" font-family="Verdana, Geneva, sans-serif" font-size="52" fill="#1F2430">${BRAND_NAME}</text>
...
if [[ -z "$BRAND_NAME" ]]; then
  echo "SKIP: branding source unavailable: $BRAND_FILE"
  exit 0
fi
...
rc=$(run_capture "$TMPDIR/second.out" bash -lc "cd '$WORK' && bash ./briterepo/bin/genpngs")
